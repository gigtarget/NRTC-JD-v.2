<script>
  ...
  async function searchItems() {
    ...
    const queries = input.split(",").map(q => q.trim()).filter(q => q); // 👈 split by commas
    ...
    queries.forEach(query => {
      ...
      if (match) {
        ...
        if (level && block && side && section) {
          ...
          messages.push(`✅ Found: ${query} (Aisle ${aisle} | Level: ${level}, ${block}, ${side} ${section})`);
        } else {
          highlightId = null;
          document.querySelectorAll(`[id^="${aisle}-"]`).forEach(el => el.classList.add("highlight"));
          messages.push(`✅ Found: ${query} (Highlighted entire Aisle ${aisle})`);
        }
        ...
        messages.push(`⚠️ Item not found: ${query}`);
      }
    });
